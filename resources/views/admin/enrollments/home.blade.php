...
@include('admin.enrollments.create')
						...
						@foreach($academics as $academic)
							@if($academic->status)
								<option class="text-danger" selected="" style="font-weight: bold;" value="{{$academic->id}}">
									{{$academic->year_start}}/{{$academic->year_end}}
...
@section('page-scripts')
	...
	<script type="text/javascript" src="{{asset("/js/enrollments/admin/home.js")}}"></script>

  @if($flash = session('message'))
    <script type="text/javascript">
        var message = "Student: <b>{{$flash}}</b> enrollment updated!";
        notify(message);
    </script>
  @endif
@endsection